...
export function ThreadToolbar({ thread }: ThreadToolbarProps): React.JSX.Element {
  ...
  return (
    <React.Fragment>
      ...
      >
        <Stack direction="row" gap="8px" sx={{ alignItems: 'center', minWidth: 0 }}>
          <AvatarGroup
            max={2}
            sx={{
              '& .MuiAvatar-root': {
          ...
          <Box sx={{ minWidth: 0 }}>
            <Typography noWrap fontSize={16} fontWeight={700} color="#fff" lineHeight="150%">
              {recipients.map((recipient) => recipient.name).join(', ')}
            </Typography>
            {thread.type === 'direct' ? (
              <Typography color="#fff" lineHeight="166%" fontSize={12}>
                Recently active
              </Typography>
            ) : null}
          </Box>
